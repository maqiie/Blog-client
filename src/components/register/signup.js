...
const Signup = ({ onToggleForm }) => {
  ...
  const handleSignup = async (e) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        if (authTokenHeader) {
          ...
          if (authToken) {
            ...
            // Use navigate to redirect to a different route
            navigate("/"); // Redirect to the dashboard or any desired route
          } else {
            setError("Invalid Authorization header format.");
          }
        } else {
      ...
      // Set loading back to false when sign-up process is completed
      setLoading(false);
    }
  };
  ...
  return (
    <div className="auth-form bg-white p-6 rounded shadow">
      <h2 className="text-2xl font-semibold mb-4 text-green-600">Sign Up</h2>
      {loading ? (
        <div className="loader4"></div> // Render the loader when loading is true
      ) : (
        <form onSubmit={handleSignup}>
          <div className="mb-4">
            <label className="block text-sm font-medium text-gray-600 mb-1">
          ...
          <button
            type="submit"
            className="bg-green-500 hover:bg-green-600 text-white py-2 px-4 rounded transition duration-300 shadow-md w-full"
          >
            Sign Up
          </button>
        </form>
